// Fork of Unity's "Hidden/Universal Render Pipeline/Terrain/Lit (Add Pass)"
// (com.unity.render-pipelines.universal Shaders/Terrain/TerrainLitAdd.shader)
// -- the shader Unity's Terrain system uses to additively draw layers 4-7,
// 8-11, etc. on top of the base 4 layers when a terrain has more than 4
// Terrain Layers. Referenced via TerrainLitStochastic.shader's own
// Dependency "AddPassShader" line instead of Unity's stock version.
//
// Exists for two reasons:
//  1. Points at the SAME local TerrainLitInputStochastic.hlsl/
//     TerrainLitPassesStochastic.hlsl as the base shader, so layers 5+ get
//     stochastic tiling breakup too (Unity's stock Add Pass shader has none
//     of that -- it's plain Unity terrain sampling).
//  2. Those shared files' HeightBasedSplatModify now reads a baked
//     _HeightBlendGlobal texture covering every layer at once, instead of
//     each pass computing its own local (and, above 4 layers, WRONG) max/
//     sum from just its own 4 masks -- see the comment on that function in
//     TerrainLitPassesStochastic.hlsl. That fix only takes effect if every
//     pass that can render a layer -- base AND this one -- reads it, which
//     is the whole reason this fork exists rather than leaving the
//     Dependency pointed at Unity's own Add Pass shader.
//
// The Properties block mirrors TerrainLitStochastic.shader's exactly:
// Unity's Terrain system feeds terrain-wide properties (_HeightTransition,
// _NumLayersCount, _HeightBlendGlobal, our per-layer Tint/Variation/
// Parallax controls) through to whichever Dependency shader is drawing,
// keyed by property name, so they need to exist here under the same names
// to actually arrive.
Shader "Custom/URP/Terrain/Lit (Stochastic Add Pass)"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #include "Assets/CLIFF/Shaders/StochasticTerrain/TerrainLitInputStochastic.hlsl"
            #include "Assets/CLIFF/Shaders/StochasticTerrain/TerrainLitPassesStochastic.hlsl"
            ENDHLSL
        }
