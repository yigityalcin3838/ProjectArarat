// Copy of URP's TerrainLitPasses.hlsl (com.unity.render-pipelines.universal
// Shaders/Terrain/TerrainLitPasses.hlsl). Only the three functions that read
// the per-layer Splat/Normal/Mask textures -- SplatmapMix, NormalMapMix,
// ComputeMasks -- have been changed to sample through
// SampleTerrainLayerStochastic (defined in TerrainLitInputStochastic.hlsl)
// instead of a plain SAMPLE_TEXTURE2D, to break up tiling repetition.
// Everything else (height-based blend, density blend, GBuffer packing,
// shadow/depth passes, instancing) is byte-for-byte Unity's own code.

#ifndef UNIVERSAL_TERRAIN_LIT_PASSES_STOCHASTIC_INCLUDED
#define UNIVERSAL_TERRAIN_LIT_PASSES_STOCHASTIC_INCLUDED
...
void NormalMapMix(float4 uvSplat01, float4 uvSplat23, inout half4 splatControl, inout half3 mixedNormal)
{
        ...
        half3 nrm = half(0.0);
        nrm += splatControl.r * UnpackNormalScale(SampleTerrainLayerStochastic(TEXTURE2D_ARGS(_Normal0, sampler_Normal0), uvSplat01.xy), _NormalScale0);
        nrm += splatControl.g * UnpackNormalScale(SampleTerrainLayerStochastic(TEXTURE2D_ARGS(_Normal1, sampler_Normal0), uvSplat01.zw), _NormalScale1);
        nrm += splatControl.b * UnpackNormalScale(SampleTerrainLayerStochastic(TEXTURE2D_ARGS(_Normal2, sampler_Normal0), uvSplat23.xy), _NormalScale2);
        nrm += splatControl.a * UnpackNormalScale(SampleTerrainLayerStochastic(TEXTURE2D_ARGS(_Normal3, sampler_Normal0), uvSplat23.zw), _NormalScale3);
...
void SplatmapMix(float4 uvMainAndLM, float4 uvSplat01, float4 uvSplat23, inout half4 splatControl, out half weight, out half4 mixedDiffuse, out half4 defaultSmoothness, inout half3 mixedNormal)
{
    ...
    diffAlbedo[0] = SampleTerrainLayerStochastic(TEXTURE2D_ARGS(_Splat0, sampler_Splat0), uvSplat01.xy);
    diffAlbedo[1] = SampleTerrainLayerStochastic(TEXTURE2D_ARGS(_Splat1, sampler_Splat0), uvSplat01.zw);
    diffAlbedo[2] = SampleTerrainLayerStochastic(TEXTURE2D_ARGS(_Splat2, sampler_Splat0), uvSplat23.xy);
    diffAlbedo[3] = SampleTerrainLayerStochastic(TEXTURE2D_ARGS(_Splat3, sampler_Splat0), uvSplat23.zw);
    ...
    // Per-layer color variation: blend each layer's two tints with a
    ...
    // Terrain Layer asset.
    mixedDiffuse = 0.0h;
    mixedDiffuse += diffAlbedo[0] * half4(_DiffuseRemapScale0.rgb * tint0 * splatControl.rrr, 1.0h);
    mixedDiffuse += diffAlbedo[1] * half4(_DiffuseRemapScale1.rgb * tint1 * splatControl.ggg, 1.0h);
    mixedDiffuse += diffAlbedo[2] * half4(_DiffuseRemapScale2.rgb * tint2 * splatControl.bbb, 1.0h);
    mixedDiffuse += diffAlbedo[3] * half4(_DiffuseRemapScale3.rgb * tint3 * splatControl.aaa, 1.0h);
...
#ifdef _MASKMAP
    masks[0] = lerp(masks[0], SampleTerrainLayerStochastic(TEXTURE2D_ARGS(_Mask0, sampler_Mask0), IN.uvSplat01.xy), hasMask.x);
    masks[1] = lerp(masks[1], SampleTerrainLayerStochastic(TEXTURE2D_ARGS(_Mask1, sampler_Mask0), IN.uvSplat01.zw), hasMask.y);
    masks[2] = lerp(masks[2], SampleTerrainLayerStochastic(TEXTURE2D_ARGS(_Mask2, sampler_Mask0), IN.uvSplat23.xy), hasMask.z);
    masks[3] = lerp(masks[3], SampleTerrainLayerStochastic(TEXTURE2D_ARGS(_Mask3, sampler_Mask0), IN.uvSplat23.zw), hasMask.w);
#endif
...
#endif
{
    ...
    SplatmapMix(IN.uvMainAndLM, uvSplat01Parallax, uvSplat23Parallax, splatControl, weight, mixedDiffuse, defaultSmoothness, normalTS);
